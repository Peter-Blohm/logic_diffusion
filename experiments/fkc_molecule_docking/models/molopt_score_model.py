...
class ScorePosNet3D(nn.Module):
    ...
    def _ito_density_step_torch(self, dt, dx, scaled_score, g, f=None, div_f=None, batch_ligand = None):
        """
        ...
        """
        # scatter(dw.squeeze(), batch_ligand).unsqueeze(1)
        B = batch_ligand.max()+1
        # Broadcast helpers
        def _to_b1(x):
            if x is None: return None
            if x.dim() == 0: return x.view(1).expand(B)
            return x
        # dt = _to_b1(dt)
        g  = _to_b1(g).to(dx.device)
        div_f = _to_b1(div_f) if div_f is not None else torch.zeros(B, device=dx.device, dtype=dx.dtype)
        if f is None:
            f = torch.zeros_like(dx)

        # Δℓ = <dx, s> − <f dt, s> − (div f) dt − ½ g^2 ||s||^2 dt

        inner = scatter((dx * scaled_score).sum(dim=1), batch_ligand)
        inner -= scatter(((f * dt) * scaled_score).sum(dim=1), batch_ligand)
        inner -= div_f.sum() * dt
        inner -= 0.5 * (g**2) * scatter(scaled_score.pow(2).sum(dim=1), batch_ligand) * dt
        return inner
    ...
    def compose_sample_diffusion(self, 
        ...
        # TODO FULL DIFFUSION INFERENCE RUN!!!
        # time sequence
        time_seq = list(reversed(range(self.num_timesteps - num_steps, self.num_timesteps)))
        t_ = 1
        dt = 1/self.num_timesteps
        ...
        lls = torch.zeros((bs,2), device=protein_pos_1.device)
        kappas = .5*torch.ones_like(batch_ligand, device=protein_pos_1.device)
        ...
        for j, i in tqdm(enumerate(time_seq), desc='sampling', total=len(time_seq)):
            ...
            if reverse_method == "baseline":
                ...
                if self.args.composition == "dombi_and":
                    score = score_1 * kappas[:, None] + score_2 * (1-kappas[:, None])
                elif self.args.composition == "poe":
                    score = score_1 + score_2
                elif self.args.composition == "dombi_contrast":
                    score_not_2 = 2*score_1 - score_2
                    score = score_1 * kappas[:, None] + score_not_2 * (1-kappas[:, None])
                else:
                    raise NotImplementedError
                beta_t = torch.tensor(self.beta(t_))
                dlog_alphadt = -(1/2) * beta_t
                ...
                eps = torch.randn_like(score)
                #TODO: here the magic seems to happen.
                if self.args.tempered_score and False:
                    dx = dt*(-f_x + 0.5*(inv_temp+1)*g**2*score) + (g/np.sqrt(inv_temp))*np.sqrt(dt)*eps*nonzero_mask
                elif self.args.composition == "dombi_and":
                    dx = dt*(-f_x + inv_temp*g**2*score) + g*np.sqrt(dt)*eps*nonzero_mask
                else:
                    dx = dt*(-f_x + inv_temp*g**2*score) + g*np.sqrt(dt)*eps*nonzero_mask
                ...
                if self.args.composition == "dombi_and" or self.args.composition == "dombi_contrast":
                    lls[:,0] += self._ito_density_step_torch(dt,dx,score_1,g,dlog_alphadt,f_x, batch_ligand)
                    lls[:,1] += self._ito_density_step_torch(dt,dx,score_2,g,dlog_alphadt,f_x, batch_ligand)
                    
                    ll_not = 2 * lls[:,0] - lls[:,1]
                    if self.args.composition == "dombi_and":
                        kappas = torch.softmax(self.args.ell*lls,dim=1)[batch_ligand,0]
                    else:
                        kappas = torch.softmax(self.args.ell*torch.stack([lls[:,0],ll_not],dim=1),dim=1)[batch_ligand,0]
                    ndim = ligand_pos.shape[1]
                    # annealing FKC terms, according to Skreta et al.
                    dw1 = (inv_temp - 1) * (ndim*dlog_alphadt + sigma ** 2 * .5 * inv_temp * (score_1 ** 2).sum(dim=1))
                    dw2 = (inv_temp - 1) * (ndim*dlog_alphadt + sigma ** 2 * .5 * inv_temp * (score_2 ** 2).sum(dim=1))
                    dw_not = sigma ** 2 * ((score_2-score_1) ** 2).sum(dim=1) + 2*dw1 - dw2
                    # power norm FKC terms
                    score_square = (score ** 2).sum(dim=1)
                    square_scores = \
                        ((score_1 ** 2).sum(dim=1)) *    kappas  + \
                        ((score_2 ** 2).sum(dim=1)) * (1-kappas)
                    
                    # ell is negative
                    if self.args.composition == "dombi_and":
                        dw = .5*(1-self.args.ell)*g**2*(score_square - square_scores) + (dw1 * kappas + dw2 * (1 - kappas))
                    else:
                        dw = .5*(1-self.args.ell)*g**2*(score_square - square_scores) + (dw1 * kappas + dw_not * (1 - kappas))
                elif self.args.composition == "poe":
                    ndim = ligand_pos.shape[1] 
                ...
                wandb.log({"w_std": torch.std(w_gen[:,0,:]).item(), "w_mean": torch.mean(w_gen[:,0,:]).item(), "ll1": torch.mean(lls[:,0]).item(), "ll2": torch.mean(lls[:,1]).item()}, step=j)
