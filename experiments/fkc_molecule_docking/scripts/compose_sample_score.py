...
if __name__ == '__main__':
    ...
    parser.add_argument('--composition', type=str, default="poe")
    parser.add_argument('--ell', type=float, default=1.)
    ...
    config.sample.composition = args.composition
    config.sample.ell = args.ell
    logger.info(config)
    ...
    exp_name += f"_compose_{config.sample.composition}"
    if config.sample.composition == "dombi_and":
        exp_name += f"{config.sample.ell}"
