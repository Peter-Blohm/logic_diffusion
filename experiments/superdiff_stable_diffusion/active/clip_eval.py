...
PATH = str(EXP_ROOT / "outputs" / "saved_sd_results") + "/"
...
def get_ll_ode(
    ...
    ll_q0_ode = (
        ...
        * (
            ...
            - torch.log(scheduler.init_noise_sigma**2)
        )
        * torch.ones((args.batch_size), device=torch_device)
    )
    ll_q0_ode -= (
        1
        / scheduler.init_noise_sigma**2
        * ((images * scheduler.init_noise_sigma) ** 2).sum((1, 2, 3))
    )
...
def get_ll_ode_guidance(
    ...
    ll_ode_obj = torch.zeros(
        (args.num_inference_steps + 1, args.batch_size), device=torch_device
    )
    ll_ode_bg = torch.zeros(
        (args.num_inference_steps + 1, args.batch_size), device=torch_device
    )
    ...
    ll_q0_ode -= (
        1
        / scheduler.init_noise_sigma**2
        * ((latents_ode * scheduler.init_noise_sigma) ** 2).sum((1, 2, 3))
    )
    ...
    for i, t in enumerate(scheduler.timesteps):  # reverse
        ...
        if obj_embeddings is not None:
            vel_obj, div_obj = get_vel(
                t, sigma, latents_ode, [obj_embeddings], eps, True
            )
        if bg_embeddings is not None:
            vel_bg, div_bg = get_vel(t, sigma, latents_ode, [bg_embeddings], eps, True)
        if uncond_embeddings is not None:
            vel_uncond, _ = get_vel(
                t, sigma, latents_ode, [uncond_embeddings], eps, False
            )
        ...
        if obj_embeddings is not None:
            ...
            dlog_q_obj += -torch.abs(dsigma) * (
                (-vel_obj / sigma) * (vel_obj - vf)
            ).sum(
                (1, 2, 3)
            )  # recall v(t) = -sigma(t) * s(t)
            ll_ode_obj[i + 1] = ll_ode_obj[i] + dlog_q_obj
        if bg_embeddings is not None:
            ll_ode_bg[i + 1] = ll_ode_bg[i] + dsigma * (
                div_bg - ((-vel_bg / sigma) * (vel_bg - vf)).sum((1, 2, 3))
            )
...
def run(args):
    if args.method == "sd_ab":
        ...
        obj_embeddings = get_text_embedding([prompt_for_sd] * args.batch_size)
    elif args.method == "without":
        prompt_for_sd = args.obj + " without " + args.bg
        obj_embeddings = get_text_embedding([prompt_for_sd] * args.batch_size)
    elif (
        args.method == "and"
        or args.method == "or"
        or args.method == "and_ode"
        or args.method == "dombi_and"
        or args.method == "dombi_contrast"
        or args.method == "and_not_superdiff"
        or args.method == "icn"
    ):
        lift = 0.0
    ...
    for i, t in tqdm(enumerate(scheduler.timesteps), colour="MAGENTA"):
        dsigma = scheduler.sigmas[i + 1] - scheduler.sigmas[i]
        sigma = scheduler.sigmas[i]
        vel_obj, dlog_obj = get_vel(t, sigma, latents, [obj_embeddings])
        ...
        if args.method in ["sd_ab", "sd_ba", "sd_ab_or", "sd_ba_or", "sd_a", "sd_b"]:
            ...
            dx_uncond = (
                2 * dsigma * vel_uncond_only
                + torch.sqrt(2 * torch.abs(dsigma) * sigma) * zs
            )
            latents_uncond += dx_uncond

            # compute SDE log-likelihood
            ll_obj[i + 1] = ll_obj[i] + (
                -torch.abs(dsigma) / sigma * (vel_obj) ** 2 - (dx * (vel_obj / sigma))
            ).sum((1, 2, 3))
            ll_bg[i + 1] = ll_obj[i + 1]

            ll_uncond[i + 1] = ll_uncond[i] + (
                -torch.abs(dsigma) / sigma * (vel_uncond_only) ** 2
                - (dx * (vel_uncond_only / sigma))
            ).sum((1, 2, 3))

            wandb.log(
                {
                    "iter": i,
                    "ll_obj": ll_obj[i + 1].mean(),
                    "ll_bg": ll_bg[i + 1].mean(),
                }
            )
            ...
            kappa[i + 1] = (
                ...
                + lift / dsigma * sigma / args.num_inference_steps
            )
            kappa[i + 1] += -(
                (vel_obj - vel_bg)
                * (vel_uncond + args.guidance_scale * (vel_bg - vel_uncond))
            ).sum((1, 2, 3))
            kappa[i + 1] /= args.guidance_scale * ((vel_obj - vel_bg) ** 2).sum(
                (1, 2, 3)
            )

            vf = vel_uncond + args.guidance_scale * (
                (vel_bg - vel_uncond)
                + kappa[i + 1][:, None, None, None] * (vel_obj - vel_bg)
            )
            latents += dsigma * vf
            ll_obj[i + 1] = ll_obj[i] + dsigma * (
                dlog_obj - ((-vel_obj / sigma) * (vel_obj - vf)).sum((1, 2, 3))
            )
            ll_bg[i + 1] = ll_bg[i] + dsigma * (
                dlog_bg - ((-vel_bg / sigma) * (vel_bg - vf)).sum((1, 2, 3))
            )
            ...
            ll_bg_temp = ll_bg[i]
            if (
                args.method == "dombi_contrast"
                or args.method == "icn"
                or args.method == "and_not_superdiff"
            ):
                gamma = args.gamma
                vel_bg_temp = vel_uncond + gamma * (vel_uncond - vel_bg)
                ll_bg_temp = ll_uncond[i] + gamma * (ll_uncond[i] - ll_bg[i])

            noise = torch.sqrt(2 * torch.abs(dsigma) * sigma) * torch.randn_like(
                latents
            )

            if args.method == "and" or args.method == "and_not_superdiff":
                dx_ind = (
                    2
                    * dsigma
                    * (vel_uncond + args.guidance_scale * (vel_bg_temp - vel_uncond))
                    + noise
                )
                kappa[i + 1] = (
                    (
                    ...
                    - (dx_ind * ((vel_obj - vel_bg_temp))).sum((1, 2, 3))
                    + sigma * lift / args.num_inference_steps
                )
                kappa[i + 1] /= (
                    2
                    * dsigma
                    * args.guidance_scale
                    * ((vel_obj - vel_bg_temp) ** 2).sum((1, 2, 3))
                )
            if args.method == "icn":  # TODO implement this baseline
                kappa[i + 1] = 0.5
            elif args.method == "dombi_and" or args.method == "dombi_contrast":
                kappa[i + 1] = torch.softmax(
                    torch.stack(
                        [args.T * (-ll_obj[i] + args.logp), args.T * (-ll_bg_temp)]
                    ),
                    0,
                )[0]
            elif args.method == "or":
                kappa[i + 1] = torch.softmax(
                    torch.stack(
                        [args.T * (ll_obj[i] + args.logp), args.T * (ll_bg[i])]
                    ),
                    0,
                )[0]

            vf = vel_uncond + args.guidance_scale * (
                (vel_bg_temp - vel_uncond)
                + kappa[i + 1][:, None, None, None] * (vel_obj - vel_bg_temp)
            )
            ...
            ll_bg[i + 1] = ll_bg[i] + (
                -torch.abs(dsigma) / sigma * (vel_bg) ** 2 - (dx * (vel_bg / sigma))
            ).sum((1, 2, 3))
            ll_uncond[i + 1] = ll_uncond[i] + (
                -torch.abs(dsigma) / sigma * (vel_uncond) ** 2
                - (dx * (vel_uncond / sigma))
            ).sum((1, 2, 3))

            diffusion_coefficient = torch.sqrt(2 * sigma)
            score = -(1.0 / sigma) * vel_obj
            base_drift_f = 2.0 * vel_obj * torch.ones(dx.shape[0], device=dx.device)
            drift_divergence_nabla_f = (
                -2.0 * dlog_obj * torch.ones(dx.shape[0], device=dx.device)
            )
            dt = torch.abs(dsigma)
            obj_density_step = full_ito_sde_step(
                dx,
                score.unsqueeze(1),
                diffusion_coefficient,
                dt * torch.ones(dx.shape[0], device=dx.device),
                base_drift_f=None,
                drift_divergence=None,
            )

            print(ll_obj[i + 1] - ll_obj[i], obj_density_step)

            wandb.log(
                {
                    "iter": i,
                    "kappa": kappa[i + 1].mean(),
                    "ll_obj": ll_obj[i + 1].mean(),
                    "ll_bg": ll_bg[i + 1].mean(),
                }
            )
            ...
            vel_bg, _ = get_vel(t, sigma, latents, [bg_embeddings])
            vf = vel_uncond + args.guidance_scale * (
                (vel_bg - vel_uncond) + kappa * (vel_obj - vel_bg)
            )
            dx = 2 * dsigma * vf + torch.sqrt(
                2 * torch.abs(dsigma) * sigma
    ...
    clip_score_min, clip_score_avg, clip_raw_scores = compute_clip_score(
        clip_processor, clip, mixed_samples, args
    )
    ...
    # cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "ImageReward")
    # image_reward = RM.load("ImageReward-v1.0", download_root=cache_dir)
    image_reward = RM.load("ImageReward-v1.0", download_root=PATH)
    image_reward_score_min, image_reward_score_avg, ir_raw_scores = (
        compute_image_reward(image_reward, mixed_samples, args)
    )
    ...
    if args.gamma != 1:
        args.method = f"{args.method}_gamma{args.gamma}"
...
def main():
    ...
    parser.add_argument(
        "--gamma", type=float, default=1.0, help="geometric average for negation"
    )
