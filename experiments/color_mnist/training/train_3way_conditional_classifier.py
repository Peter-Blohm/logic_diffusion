...
score_model1.load_state_dict(torch.load(CHECKPOINTS_DIR / 'gen_MN1_ckpt_195.pth', map_location=device))
...
score_model2.load_state_dict(torch.load(CHECKPOINTS_DIR / 'gen_MN2_ckpt_195.pth', map_location=device))
...
score_model3.load_state_dict(torch.load(CHECKPOINTS_DIR / 'gen_MN3_ckpt_195.pth', map_location=device))
...
cls_ckpt = torch.load(CHECKPOINTS_DIR / '3way_classifier_ckpt_700.pth', map_location=device)
...
for epoch in range(1, 200 + 1):
    ...
    if epoch % 20 == 0:
        torch.save(model.state_dict(), CHECKPOINTS_DIR / ('3way_conditional_classifier_ckpt_' + str(epoch) + '.pth'))
