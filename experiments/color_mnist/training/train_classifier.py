...
score_model1.load_state_dict(torch.load(CHECKPOINTS_DIR / 'gen_M1_ckpt_195.pth', map_location=device))
...
score_model2.load_state_dict(torch.load(CHECKPOINTS_DIR / 'gen_M2_ckpt_195.pth', map_location=device))
...
for epoch in range(1, 200 + 1):
    ...
    if epoch % 25 == 0:
       test(model, score_model1, score_model2, 300, num_steps, device)
       torch.save(model.state_dict(), CHECKPOINTS_DIR / ('classifier_ckpt_' + str(epoch) + '.pth'))
